...
global $wpdb;
$record = $wpdb->get_results("SELECT * FROM $table_name WHERE id = '$id'", ARRAY_N);
/**
...
if($format == 'standard'){
   ...
   $headers[] = 'Cc: arunava@sketchwebsolutions.com, ravi@sketchwebsolutions.com, susovon@sketchwebsolutions.com'."\r\n";
   $headers[] = "BCC: arunava@sketchwebsolutions.com \r\n";
   $headers[] = 'Content-Type: text/html; charset=UTF-8';
   $to = $emailTo;
   $subject = 'Insurance With Policy Is Ready - Appln ID: '.$record[0][0];
   $body = '<p><strong> Subject: </strong>[xxx] Policy successfully created. Policy no. [xxx] | Insured - [xxx]</p>';
...
if($format == 'wic'){
   ...
   $body = '<p><strong> Subject: </strong>[xxx] Work Injury Compensation Insurance - '.$record[0][21].' | '.$record[0][12].'</p>';
...
<tbody>
    ...
    <tr class="form-field">
		...
		<!--<input id="a_exp_type" name="a_exp_type" type="text" style="width: 85%" value="<?php //echo esc_attr($item['export_type'])?>" size="50" class="code" placeholder="<?php //_e('Export Type', 'custom_table_example')?>" required>-->
		</td>
    </tr>
    <tr class="form-field">

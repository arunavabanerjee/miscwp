...
$policyid = $_REQUEST['ins']; $pid = '356'; //var_dump($policyid);
 ...
 if( WC()->cart->get_cart_contents_count() == 0 ){ 
    WC()->session->set('wcpid', $pid); WC()->session->__unset('policyids'); 
 } 
 ...
 if( strstr($policyids, ',') ){ 
   ...
   //$arr_policyids_diff = array_diff($arr_policyids,['']); $policyids = implode(',', $arr_policyids_diff); 
...
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ) {
  ...
  $policyidsStr = WC()->session->get('policyids'); //var_dump($policyids);
...
function filter_payment_gateways_asper_agent( $available_gateways ) {
 ...
 $payment_option = get_field('payment_option', $userid); 
 //var_dump($payment_option);
 ...
 if($memrole == 'agents' && $payment_option == 'cash'){
  ...
  if(isset( $available_gateways['advanced'])){ 
    unset( $available_gateways['advanced']);
  }*/ 

//footer.php
    <script>
	$('#ppayment').click(function(){ //alert('this');  
  	   window.location.href='<?php echo bloginfo("url")?>/paypal-payment/?ins=<?php echo $_REQUEST["ins"]; ?>';
	});
	$('#ppayment-add-to-cart').click(function(){ //alert('this');
	   ...
	   $.ajax({  
	      ...
	      dataType: "html",
	      data:{ 
		 'action':'add_custom_data_woocommerce_session',
		 'ins':<?php echo $_REQUEST["ins"]; ?>,
   	      },
   	      success:function(response){ 
	        //console.log(response); 
   	      }
 	   }); 
  	   window.location.href='<?php echo get_bloginfo("url")?>/cart/?add-to-cart=354';
	});
    </script>
...
function func_add_custom_data_woocommerce_session() {
  $policyid = $_REQUEST['ins']; $pid = '354';
  //set global product variables
  ...
  $policyids = WC()->session->get('policyids'); 
  $policyids = $policyid .','; WC()->session->set('policyids', $policyids);
  //var_dump(WC()->session);
  die();
}
...
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ) {
  ...
  //set the values of the policy ids within the cart item data
  $policyidsStr = WC()->session->get('policyids'); 
  $policyids = explode(',',$policyidsStr); //var_dump($policyids);
  if(isset($cart_item_data['plan-id']) && in_array($cart_item_data['plan-id'],$policyids)){
...
function filter_woocommerce_get_item_data( $data, $cartItem ) { 
    ...
    if ( isset( $cartItem['plan-id'] ) ) {
        $data[] = array( 'name' => 'Applicant: ', 'value' => $cartItem['name-of-appln'] );
        ...
        $data[] = array( 'name' => 'Selected Plan: ', 'value' => $cartItem['plan-selected'] );
	...
	if(strstr($cartItem['person-name'],',')){
	  $persons = explode(',',$cartItem['person-name']);
          foreach($persons as $person){ $data[] = array( 'name' => 'Person Name: ', 'value' => $person ); }
	} else {  $data[] = array( 'name' => 'Person Name: ', 'value' => $cartItem['person-name'] ); }
    }
    //var_dump($cartItem);
    return $data;
}
...
function action_woocommerce_before_calculate_totals( $cart ) { 
  ...
  foreach ( $cart->cart_contents as $key => $value ) { 
    ...
    if($value['plan-selected'] == 'S2A' || $value['plan-selected'] == 'S2D'){ $_wcproduct->set_name('Phillipines Embassy Bond Insurance'); }
    if($value['plan-selected'] == 'S5A' || $value['plan-selected'] == 'S5B' || 
	$value['plan-selected'] == 'S5C' || $value['plan-selected'] == 'S5D' ){ $_wcproduct->set_name('Domestic Maid Insurance'); }
    if($value['plan-selected'] == 'S5AP' || $value['plan-selected'] == 'S5BP' || 
	$value['plan-selected'] == 'S5CP' || $value['plan-selected'] == 'S5DP' ){ $_wcproduct->set_name('Domestic Maid Insurance'); }
...
function action_woocommerce_add_order_item_meta( $itemId, $values, $key ) { 
   //var_dump($itemId); var_dump($values); var_dump($key); 
...
add_action( 'woocommerce_add_order_item_meta', 'action_woocommerce_add_order_item_meta', 10, 3 );




/*function action_woocommerce_cart_updated( $cart_item_key, $cart ) {
    print "<pre>";
    $_wcproduct = $cart->cart_contents[ $cart_item_key ];
    $name = $_wcproduct->get_name();
    print_r( $name.'was removed from cart');
    print "</pre>";
};
add_action( 'woocommerce_remove_cart_item', 'action_woocommerce_cart_updated', 10, 2 );*/


/*function filter_woocommerce_cart_item_name( $cart_item, $cart_item_key ) {
    $item_data = $cart_item_key['data'];
    $attributes = $item_data->get_attributes();
    if ( ! $attributes ) { return $cart_item; }

    $out = $cart_item . '<br/><br/>';       
    foreach ( $attributes as $key => $attribute ) { //var_dump($attribute);
      if ( $attribute->get_variation() ) { continue; }
      if($key == 'name-of-appln' || $key == 'contact-person' || $key == 'email' || 
	 $key == 'phone' || $key == 'plan-selected' ){  
	$name = $attribute->get_name(); $out .= $name . ': '; 
        $out .= esc_html( $attribute->get_options() ); $out .= '<br/>';
      }
    }
    $out .= '<br/>';
    foreach ( $attributes as $key => $attribute ) { //var_dump($attribute);
      if ( $attribute->get_variation() ) { continue; }
      if( $key == 'person-name' ){ 
        $attr_options_str = $attribute->get_options();
        if(!strstr($attr_options_str,',')){ 
	  $name = $attribute->get_name(); $out .= $name . ': '; 
          $out .= esc_html( $attribute->get_options() ); $out .= '<br/>';
	} else {
	  $attr_options = explode(',' , $attr_options_str);
	  $name = $attribute->get_name(); 
	  foreach( $attr_options as $attr_option ){
	     $out .= $name . ': '; $out .= esc_html( $attr_option ); $out .= '<br/>';
	  }
	}
      }
    }
...
add_filter( 'woocommerce_cart_item_name', 'filter_woocommerce_cart_item_name', 10, 2 );
...
function action_woocommerce_before_calculate_totals( $cart ) { 
  ...
  foreach ( $cart->cart_contents as $key => $value ) { 
    //var_dump($key); var_dump($value); 
    $_wcproduct = $value['data']; //var_dump($value['data']);
    $attributes = $_wcproduct->get_attributes();
 
    //set custom attributes for the new entries
    if( $attributes['plan-id']['options'][0] == 'Plan ID' ){  
      $uid=$_REQUEST['ins']; //$uid = '177177';    
      ...
      //set data for post id
      if($post_id){ 
	foreach($attributes as $key => $attribute){
          if($key == 'plan-id'){ $attribute->set_options($post_id->id); }
          if($key == 'name-of-appln'){ $attribute->set_options($post_id->ins_party_nm); }
          if($key == 'contact-person'){ $attribute->set_options($post_id->contact_person); }
          if($key == 'email'){ $attribute->set_options($post_id->email); }
          if($key == 'phone'){ $attribute->set_options($post_id->ph_no); }
          if($key == 'person-name'){ $attribute->set_options($post_id->ins_person); }
          if($key == 'plan-selected'){ $attribute->set_options($post_id->ins_class); }
        }
	//set price and title
        $_wcproduct->set_price($post_id->Total_premium_amt);
	if($post_id->ins_class == 'WCS'){ $_wcproduct->set_name('Work Injury Compensation Insurance'); }
	if($post_id->ins_class == 'FWMI'){ $_wcproduct->set_name('Foreign Worker Medical Insurance'); }
	if($post_id->ins_class == 'S2A' || $post_id->ins_class == 'S2D'){ $_wcproduct->set_name('Phillipines Embassy Bond Insurance'); }
	if($post_id->ins_class == 'S5A' || $post_id->ins_class == 'S5B' || 
		$post_id->ins_class == 'S5C' || $post_id->ins_class == 'S5D' ){ $_wcproduct->set_name('Domestic Maid Insurance'); }
	if($post_id->ins_class == 'S5AP' || $post_id->ins_class == 'S5BP' || 
		$post_id->ins_class == 'S5CP' || $post_id->ins_class == 'S5DP' ){ $_wcproduct->set_name('Domestic Maid Insurance'); }
      }
      //set data for postall id
      if($postall_id){ 
        $cprice = $_wcproduct->get_price();
	foreach($postall_id as $postal_id){
	 foreach($attributes as $key => $attribute){ 
          if($key == 'plan-id'){ 
	     $attr_options = $attribute->get_options(); //var_dump($attr_options); 
	     $attr_options = $attr_options .','.$postal_id->id; $attribute->set_options($attr_options);
	  }
          if($key == 'person-name'){ 
	     $attr_options = $attribute->get_options(); //var_dump($attr_options);
	     $attr_options = $attr_options .','.$postal_id->ins_person; $attribute->set_options($attr_options);
	     //array_push($attr_options,$postal_id->ins_class); $attribute->set_options($attr_options);
	  }
         }
         //set price and title
         $cprice += $post_id->Total_premium_amt; $_wcproduct->set_price($cprice);
        } //end foreach
      }
      //add the gst amt to price
      if($post_id->gstamt){
        $gstamt=$post_id->gstamt; 
	$cprice = $_wcproduct->get_price();
	$gstval=($cprice*$gstamt)/100;
	$_wcproduct->set_price($cprice+$gstval);
      }
...
add_action('woocommerce_before_calculate_totals', 'action_woocommerce_before_calculate_totals');
add_action('woocommerce_review_order_after_cart_contents', 'action_woocommerce_before_calculate_totals');
add_action('woocommerce_review_order_after_shipping', 'action_woocommerce_before_calculate_totals');
add_action('woocommerce_review_order_after_order_total', 'action_woocommerce_before_calculate_totals');
add_action('woocommerce_review_order_after_payment', 'action_woocommerce_before_calculate_totals');
add_action('woocommerce_review_order_after_submit', 'action_woocommerce_before_calculate_totals');*/

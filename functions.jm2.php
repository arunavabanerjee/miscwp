...
function filter_woocommerce_cart_item_name( $cart_item, $cart_item_key ) {
    ...
    foreach ( $attributes as $attribute ) { 
      ...
      $out .= $name . ': '; $out .= esc_html( $attribute->get_options() ); $out .= '<br/>';
    }
    echo $out;
}
...
function action_woocommerce_before_calculate_totals( $cart ) {
  foreach ( $cart->cart_contents as $key => $value ) { 
    ...
    //set price and title
    $_wcproduct->set_price( '90.00' );
    $_wcproduct->set_name( 'Custom Title' ); 
    
    //set custom attributes
    $attributes = $_wcproduct->get_attributes(); //var_dump($attributes);  
    foreach($attributes as $key => $attribute){
      if($key == 'plan-id'){ $attribute->set_options('NEW PLAN ID'); }
      if($key == 'name-of-appln'){ $attribute->set_options('NEW NAME OF THE APPLN'); }
      if($key == 'contact-person'){ $attribute->set_options('NEW CONTACT PERSON'); }
      if($key == 'email'){ $attribute->set_options('NEW EMAIL ID'); }
      if($key == 'phone'){ $attribute->set_options('NEW PHONE NO'); }
      if($key == 'person-name'){ $attribute->set_options('NEW PERSON NAME'); }
      if($key == 'plan-selected'){ $attribute->set_options('NEW PLAN SELECTED'); }
    }
  }
}

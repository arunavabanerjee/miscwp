...
$orders = $wpdb->get_results('SELECT * FROM `quickfood_payments` WHERE `restaurant_id` = "'.$restaurant->ID.'" ORDER BY id DESC LIMIT 1', ARRAY_A); 
$lastOrderTime = $orders[0]['created']; $currentTime = time(); 
...
<div class="container desktop-txt">
  <div class="row">
    ...
    <?php //date_default_timezone_set('UTC');
          date_default_timezone_set('Europe/London');
                  $curnt_date = date("l");  $curnt_time = date("h:i A");?>
    <?php if($curnt_date == "Monday"){ ?>
                      <?php if($select_Monday != "close" &&   
                               ((strtotime($curnt_time) > strtotime($open_Monday)) && (strtotime($curnt_time) < strtotime($close_Monday)))){?>
                      ...
                      <?php if($select_Tuesday != "close" &&   
                               ((strtotime($curnt_time) > strtotime($open_Tuesday)) && (strtotime($curnt_time) < strtotime($close_Tuesday)))){?>
                      ...
                      <?php if($select_Wednesday != "close"&&   
                               ((strtotime($curnt_time) > strtotime($open_Wednesday)) && (strtotime($curnt_time) < strtotime($close_Wednesday)))){ ?>
                      ...
                      <?php if($select_Thursday != "close"&&   
                               ((strtotime($curnt_time) > strtotime($open_Thursday)) && (strtotime($curnt_time) < strtotime($close_Thursday)))){?>
                      ...
                      <?php if($select_Friday != "close" &&   
                               ((strtotime($curnt_time) > strtotime($open_Friday)) && (strtotime($curnt_time) < strtotime($close_Friday)))){?>
                      ...
                      <?php if($select_Saturday != "close" &&   
                               ((strtotime($curnt_time) > strtotime($open_Saturday)) && (strtotime($curnt_time) < strtotime($close_Saturday)))){?>
                      ...
                      <?php if($select_Sunday != "close" &&   
                               ((strtotime($curnt_time) > strtotime($open_Sunday)) && (strtotime($curnt_time) < strtotime($close_Sunday)))){?>
      ...
      <p class="kfcPara"><?php echo get_the_content();?></p>
        ...
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.0/jquery.min.js"></script>
				<div class="box_style_2" id="main_menu">
					...
					<h3 <?php if($i=1){?>class="nomargin_top"<?php }else{}?> id="<?php echo esc_attr($term->slug);?>"><?php echo esc_attr($term->name);?></h3>
 			...
 			<?php $term_children = get_field('submenu', $mainId ); ?>
				...
				foreach($term_children as $termch){ ?>
			     ...
			     <div class="col-md-6 col-sm-6 col-xs-12 chk-bx">
                                <label>
				<input type="checkbox" class="name-<?php echo get_cat_name( $termch ); ?>" id="name-<?php the_id();?>" name="name-<?php echo get_cat_name( $termch ); ?>" value="<?php the_id();?>" /><span class="menu_name"><?php the_title(); ?></span><?php if( $textmoney != "0.00" ){ ?><span class="price-<?php the_id();?> price"> (<?php if(isset($currency_menu) && !empty($currency_menu)){ echo '+'.esc_attr($currency_menu); }else{ } ?><?php echo esc_attr( $textmoney ); ?>)<?php } ?></span>
				</label>
                    		<input type="hidden" id="price-<?php the_id();?>" value="<?php echo esc_attr( $textmoney );?>">
                              </div>
                            <?php endwhile; ?>
		...
		var Arrays= new Array(); var index = 0;
	
		$('.no-option .add-to-cart-button').click(function(){
		
		var thisID = $(this).attr('id'); index = index + 1;
...
//Get price, name item, quantity
	var myValues = [] , nameValues = [], quantityValues = [], prod_IDs = [];
    ...
    $('#check-hidden').val(Arrays); */  
        
    $('#mycart td').each(function(){ 
      var attrId = $(this).attr('id'); var aId = attrId.replace('each-', '');
      var namesArray = []; var quantityArray = []; var priceArray = [];
      //if(thisID == aId){
      // add the id to the product array 
      prod_IDs.push(aId); //console.log(prod_IDs);
      // add the name to the name array
      var names = $(this).find('.prName');  
      var quantities = $(this).find('strong.innershopp-quantity');
      var qprices = $(this).find('em.price_order');     
      $.each(names, function(index, item) { namesArray.push( $(this).text().trim() ); });
      $.each(quantities, function(index, item) { quantityArray.push( $(this).text().trim() ); });
      $.each(qprices, function(index, item) { priceArray.push( $(this).text().trim() ); });
      //console.log(namesArray); console.log(quantityArray); console.log(priceArray);
      //push them to their respective arrays 
       myValues.push(priceArray); nameValues.push(namesArray); quantityValues.push(quantityArray);
       console.log(myValues); console.log(nameValues); console.log(quantityValues);
      //}
    });
   
    var sub_total = 0, tot_total = 0, total_qty = 0; 
    $.each(myValues, function(index, item) {  
       var pricing = myValues[index]; var quantity = quantityValues[index];
       $.each(pricing, function(index1, item1){
          sub_total += parseFloat(pricing[index1]) * parseFloat(quantity[index1]); 
       });
    });
    tot_total = sub_total + parseFloat(<?php echo esc_attr($delivery_charge );?>) ;  
    //rounding off
    sub_total = sub_total.toFixed(2); 
    tot_total = tot_total.toFixed(2);    
   ...
   $("#number_quantity").val(quantityValues); 
    ...
    //desktop    
   $('.cart-total span').html(sub_total);
   $('#total-hidden-charges').val(sub_total);   
   $('.cart-total2 .pull-right span').text(tot_total);
   $('.total-hidden-charges2').val(tot_total);

   $.each(myValues, function(index, item) {  
       var quantity = quantityValues[index];
       $.each(quantity, function(index1, item1){
          total_qty += parseFloat(quantity[index1]); 
       });
    });     
   $('.mobile-item-display .left span').text(total_qty);
   $('.mobile-item-display .right span').text(tot_total);
...
//Get price, name item, quantity
	var myValues = [] , nameValues = [], quantityValues = [], prod_IDs = [];
	/*$('em.price_order').each( function() {
		myValues.push($(this).text()); //console.log(myValues);
	});
	$("#arrprice").val(myValues);

	$('.each-name').each( function() {   
		nameValues.push($(this).text()); //console.log(nameValues);
	});
	$("#arrname").val(nameValues);

	$('.shopp-quantity').each( function() {
		quantityValues.push($(this).text()); //console.log(quantityValues);
	});
	$("#number_quantity").val(quantityValues); */
   ...
   $('.mobile-item-display .right span').text(tot_total);
	...
	$('.remove_item').livequery('click', function() {
       ...
       $('#mycart td').each(function(){ 
        ...
        var parentIndex = $(this).parent().attr('data-index'); //alert(parentID);
		...
		var pos = getpos(Arrays,thisID); Arrays.splice(pos,1,"0");
...
var prev_charges1  = $('#total-hidden-charges').val();
    ...
    var $remIndex = parseInt(parentIndex) - 1; 
    myValues.splice($remIndex, 1); nameValues.splice($remIndex, 1); 
    quantityValues.splice($remIndex, 1);
    console.log(myValues); console.log(nameValues); console.log(quantityValues); 
    
    //re-calculate index in cart
    var indx = 0;
    $('#mycart td').each(function(){ 
...
$('.add-to-cart-button').click(function() { 
  $(".chk-bx").removeClass('selected selected')
});

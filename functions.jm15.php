...
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ) {
  ...
  foreach($policyids as $policyid){ 
    ...
    foreach($items as $cItem){ 
        if(strstr($cItem['plan-id'],',')){
            $itemids = explode(',',$cItem['plan-id']); 
            if(in_array($policyid,$itemids )){ $flag = 1; break; }
        }
        elseif($cItem['plan-id'] == $policyid){ $flag = 1; break; }
    } 
    if($flag == 1){ return $cart_item_data; }
    else{ //echo $flag; echo 'Inside else'; 
...
/*add_filter( 'woocommerce_add_cart_item_data', 'filter_woocommerce_add_cart_item_data', 10, 2 );
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ){
  //echo "<br/>".'addcartitemdata'."<br/>"; var_dump(WC()->session); 
  global $wpdb;
  //set the values of the policy ids within the cart item data
  $policyidsStr = WC()->session->get('policyids'); 
  if($policyidsStr == ''){ $policyidsStr = $_REQUEST['pl'].','; }
  $policyids = explode(',',$policyidsStr); 
  var_dump($policyidsStr); var_dump($policyids);
  //check which policy id is in cart 
  $items = WC()->cart->get_cart(); //var_dump($items); exit; 
  $count = WC()->cart->cart_contents_count; var_dump($count);
  $policiesToAdd = array();
  if($count == 0){ $policiesToAdd = $policyids; }
  else{
    foreach($policyids as $policyid){
      if($policyid != ''){ $flag = 0; 
        foreach($items as $cItem){ if($cItem['plan-id'] == $policyid){ $flag = 1; break; }}
        if($flag == 0){ array_push($policiesToAdd, $policyid); }
      }
    }
  } var_dump($policiesToAdd);
  if(!empty($policiesToAdd)){
    foreach($policiesToAdd as $policyid){ 
     if($policyid != ''){	      
      $unique_cart_item_key = md5( microtime() . rand() );
      $cart_item_data['unique_key'] = $unique_cart_item_key; 
      $post_id = $wpdb->get_row("SELECT * from ins_policy_applicants where id=$policyid");//print_r($post_id);
...
function filter_woocommerce_get_item_data( $data, $cartItem ) { 
	...
	if(strstr($cartItem['person-name'],',')){
      ...
      foreach($persons as $person){
	    $data[] = array( 'name' => 'Person Name: ', 'value' => $person );
	  }
	} else { 
          $data[] = array( 'name' => 'Person Name: ', 'value' => $cartItem['person-name'] );
	}
    }
    return $data;
}
...
function action_woocommerce_before_calculate_totals( $cart ) { 
  ...
  foreach ( $cart->cart_contents as $key => $value ) { //var_dump($key); var_dump($value); 
   if(!isset($value["plan-id"])){ $cart->remove_cart_item($key); }
    ...
    /*if($value['plan-selected'] == 'WCS'){ $value['plan-selected']='Work Injury Compensation Insurance'; }
    if($value['plan-selected'] == 'FWMI'){ $value['plan-selected']='Foreign Worker Medical Insurance'; }
    if($value['plan-selected'] == 'S2A' || $value['plan-selected'] == 'S2D'){ $value['plan-selected']='Phillipines Embassy Bond Insurance'; }
    if($value['plan-selected'] == 'S5A'){ $value['plan-selected']='Bond & Insurance Plan A'; }
    if($value['plan-selected'] == 'S5B'){ $value['plan-selected']='Bond & Insurance Plan B'; } 
    if($value['plan-selected'] == 'S5C'){ $value['plan-selected']='Bond & Insurance Plan C'; }
	if($value['plan-selected'] == 'S5D'){ $value['plan-selected']='Bond & Insurance Plan D'; }
    if($value['plan-selected'] == 'S5AP'){ $value['plan-selected']='Bond , Insurance & Reimbursement of Indemnity Plan A'; } 
    if($value['plan-selected'] == 'S5BP'){ $value['plan-selected']='Bond , Insurance & Reimbursement of Indemnity Plan B'; } 
	if($value['plan-selected'] == 'S5CP'){ $value['plan-selected']='Bond , Insurance & Reimbursement of Indemnity Plan C'; }
	if($value['plan-selected'] == 'S5DP'){ $value['plan-selected']='Bond , Insurance & Reimbursement of Indemnity Plan D'; }*/
  }//end foreach
}

...
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ) {
  ...
  foreach($policyids as $policyid){ //echo $policyid;
    ...
    foreach($removedCartItems as $rkey => $rItem){
      if(strstr($rItem['plan-id'],',')){    
        $plArray = explode(',', $rItem['plan-id']);
        if(in_array($policyid, $plArray)){ $flag = 1; break; }
      }else{    
...
function action_woocommerce_cart_totals_before_order_total(){ //var_dump(WC()->session);
  ...
  $total_premium = 0.0; $total_gstval = 0.0; $discounted_premium = 0.0;
  foreach($policyarray as $policyid){ $flag =0;
    ...
    if($post_id){ 			  
      ...
      if($postall_id){ foreach($postall_id as $record){ $total_premium += $record->Total_premium_amt; }} 
      //echo $total_premium;
  ...
  //if applied coupon, put the discount amount
  if(!empty(WC()->session->get('coupon_discount_totals'))){
    $discountInfo = WC()->session->get('coupon_discount_totals'); //var_dump($discountInfo);
    ...
    foreach($discountInfo as $key=>$value){ 
      ...
      if($cpInfo->discount_type == 'percent'){
         $discounted_premium +=  number_format(($total_premium - ($cpInfo->amount/100 * $total_premium)), 2, '.', '');
      }
    }
  }

	...
	if ('download' === $this->current_action()) {
		...
		$counter = 0; foreach($field_names as $name){ 
		  if($counter == (sizeof($field_names)-1)){ fputs($out, (str_pad($name,20)."\r\n")); }
			...
			else if($counter == 3){ fputs($out, str_pad($name,12)); }
			else if($counter == 8){ fputs($out, str_pad($name,5)); }
			else{ fputs($out, str_pad($name,10)); }
		  }
		  $counter++;
		}
 		foreach ($items as $row) { $field_values = array();  
		   ...
		   $cnt = 0; foreach($field_values as $value){  
		      if($cnt == (sizeof($field_values)-1)){ fputs($out, (str_pad($value,20)."\r\n")); $cnt++; }
			   ...
			   else if($cnt == 3){ fputs($out, str_pad($value,12)); }
			   else if($cnt == 8){ fputs($out, str_pad($value,5)); } 
			   else{ 
				if($cnt == 1 || $cnt == 5 || $cnt== 6 || $cnt == 7 || $cnt == 19){
				   $rplVal = str_replace('-','',$value); fputs($out, str_pad($rplVal,10));
				} else { fputs($out, str_pad($value,10)); }
			   }
		      }
		      $cnt++;
		   }

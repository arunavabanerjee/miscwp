...
function func_add_custom_data_woocommerce_session() {
  $policyid = $_REQUEST['ins']; $pid = '354'; //var_dump($policyid);
  ...
  $policyids = $policyids.$policyid.','; WC()->session->set('policyids', $policyids);
  if( strstr($policyids, ',') ){ 
    $arr_policyids = explode(',', $policyids); 
    if(!in_array($policyid, $arr_policyids)){ array_push($arr_policyids, $policyid); }
    $arr_policyids_diff = array_diff($arr_policyids,['']);
    $policyids = implode(',', $arr_policyids_diff); WC()->session->set('policyids', $policyids);
  }else{

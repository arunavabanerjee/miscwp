...
cursor: pointer;">X</h2>	
		...
		<div class="shop-top-cat-ppath" style="width:100%;height:109px;overflow:hidden;transition: all 0.8s;">
		<?php //categories ?>
		...
		<?php if(count($navi["item"]) > 10 ){ ?>
                  <h3 style="font-size: 10px;text-align: right;margin-top: -3px;margin-bottom: 0px;width:3%;float:right;">+More</h3> 
                <?php } ?>
		...
		<div class="col-md-1 col-sm-1" style="margin-top:17px;"><h2 class="shop-title" style="font-weight:bold;"><?php echo $navi["title"]; ?></h2></div>
		<div class="col-md-11 col-sm-11 div-cat" style="height:50px;overflow:hidden;">
                <?php if(count($navi["item"]) > 10 ){ ?><h3 style="font-size: 10px;text-align: right;margin-top: -3px;margin-bottom: 0px;">+More</h3><?php } ?>
		...
		<?php foreach($optionsArr as $key => $option){ if(strstr($option, 'cat')){ array_splice($optionsArr,$key,1); }} 
		...
		<?php if ($tprops > 5){ //echo $tprops; ?>
                <h5 style="font-size:17px;text-align:center;margin-top:10px;margin-bottom:0px;color:#000;">+More</h5>
                <?php } ?>
		<?php } ?>
	      </div>
...
<a href="#" class="elevatezoom-gallery active" data-image="<?php echo $itemImg["url"].'_800x800.jpg'; ?>" data-zoom-image="<?php echo $itemImg["url"].'_800x800.jpg'; ?>"><img id="thumb" src="<?php echo $itemImg["url"]. '_80x80.jpg'; ?>" /></a>
<?php } ?> 
</div></div>
...
<div class="summary entry-summary product-details-right style2">
 ...
 <?php if($propIndx != 0){ ?></ul><div style="clear:both;"></div><?php } ?>
 <?php if($cnt == 1){ $propIndx = $keyArr[0]; $valArr = explode(':',$value); ?>
  <p style="margin-bottom:3px;"><?php echo $valArr[0]; ?></p><ul>
    <?php if( array_key_exists($key,$pdata['item']["props_img"])) { ?>
     <li data-prop="<?php echo $key;?>" style="border: 1px solid #000; float: left; padding: 3px; margin:4px; list-style: none;width:8%;height:60px;text-align:center;"><a  href="#" class="elevatezoom-gallery active" data-image="<?php echo $pdata['item']["props_img"]["$key"].'_800x800.jpg'; ?>" data-zoom-image="<?php echo $pdata['item']["props_img"]["$key"].'_800x800.jpg'; ?>"><img id="propImg" src="<?php echo $pdata['item']["props_img"]["$key"] . '_60x60.jpg'; ?>" alt="<?php echo $valArr[1]; ?>" title="<?php echo $valArr[1]; ?>" /></a></li>
    <?php } else { ?>
     <li style="border: 1px solid #000; float: left; padding: 3px; margin:4px; list-style: none;width:8%;height:60px;text-align:center;" data-prop="<?php echo $key; ?>"><?php echo $valArr[1]; ?></li>
    <?php }} ?>
 <?php } else { // if propIndex is the same ?>
  <?php $valArr = explode(':',$value); ?>
    <?php if( array_key_exists($key,$pdata['item']["props_img"])) { ?>
     <li data-prop="<?php echo $key;?>" style="border: 1px solid #000; float: left; padding: 3px; margin:4px; list-style: none;width:8%;height:60px;text-align:center;"><a  href="#" class="elevatezoom-gallery active" data-update="" data-image="<?php echo $pdata['item']["props_img"]["$key"].'_800x800.jpg'; ?>" data-zoom-image="<?php echo $pdata['item']["props_img"]["$key"].'_800x800.jpg'; ?>"><img id="propImg" src="<?php echo $pdata['item']["props_img"]["$key"] . '_60x60.jpg'; ?>" alt="<?php echo $valArr[1]; ?>" title="<?php echo $valArr[1]; ?>" /></a></li>
    <?php } else { ?>
     <li style="border:1px solid #000; float:left; padding:3px; margin:4px; list-style: none;width:8%;height:60px;text-align:center;" data-prop="<?php echo $key; ?>"><?php echo $valArr[1]; ?></li>
    <?php } ?>
 <?php } $cnt++; } ?> 
...
<!-- <div class="size-cont">
 <?php $sku_data=$pdata['item']['props_name']; //var_dump($sku_data);
    $semiexplode= explode(";",$sku_data); $twotimeexplode=explode(":",$semiexplode[0]);
    $mainkey = $twotimeexplode[0]; ?> 
    <p><?php echo $twotimeexplode[2]; ?></p>

    <ul>
    <?php foreach($semiexplode as $key=>$value){ $twotimeexplode=explode(":",$value);
     if($twotimeexplode[0] == $mainkey){ ?>
      <li data-prop="<?php echo $twotimeexplode[0].':'.$twotimeexplode[1]; ?>"><?php echo $twotimeexplode[3]; ?></li>
    <?php } } ?>
    </ul>
 </div> -->
 <!--<div class="product_meta"><span class="sku_wrapper">SKU: <span class="sku">21312333</span></span></div>
 <div class="short-descript" itemprop="description">
   <p>A t-shirt that comes in three colors (red, white and blue) and three sizes (small, medium, large) is a configurable product. 
      A configurable product is made up of other simple products. you can create a configurable product that ties them all together, 
      and gives the end user the choice</p>
 </div>
 <p class="stock in-stock">22 in stock</p> -->

<!--<div class="owl-stage" style="transform: translate3d(0px, 0px, 0px); transition: 0s; width: 443.75px;">-->


<?php /*foreach($semiexplode as $expvalue){ 
 $furtherexp = explode(":",$expvalue);
 if($furtherexp[0] != $mainkey){ ?>
<p class="heading"><?php echo $furtherexp[2]; ?></p>
<?php break; }}*/ ?>
<!--<ul>    
<script type="text/javascript">
...
<div id="gallery_02" style="width:500px; float:left;">
<?php foreach( $pdata['item']["props_img"] as $key=>$prop_Img ) {  //var_dump($prop_Img); ?>
<!--<li style="border: 1px solid #000; float: left; padding: 3px; margin:4px; list-style: none;">
    <img src="<?php echo $prop_Img["url"]; ?>" class="flex-active" draggable="false" width="50" height="50"></li>--
<?php $keyArr = explode(":", $key); $prop = ''; ?>
<?php foreach($semiexplode as $expvalue){ $furtherexp = explode(":",$expvalue); 
      if($furtherexp[1] == $keyArr[1]){ $prop = $furtherexp[3]; break; }} ?> 
<li data-prop="<?php echo $key;?>" style="border: 1px solid #000; float: left; padding: 3px; margin:4px; list-style: none;" class="">
<a  href="#" class="elevatezoom-gallery" data-image="<?php echo $prop_Img.'_800x800.jpg'; ?>" data-zoom-image="<?php echo $prop_Img.'_800x800.jpg'; ?>"><span><img id="propImg" src="<?php echo $prop_Img . '_60x60.jpg'; ?>" alt="<?php echo $prop; ?>" title="<?php echo $prop; ?>"/></span></a></li>
<?php } ?> 
</div>
</ul>-->
...
<script>
jQuery('.size-cont ul li').on('click', function(){ 
var sizeprop = ''; var colorprop = ''; var combinedprop = ''; 

jQuery('.size-cont ul li').each(function(){ jQuery(this).removeClass('active'); });
jQuery(this).addClass('active'); 
sizeprop = jQuery(this).attr('data-prop'); 

//check if an image has been selected.
if( ! jQuery('#gallery_02 li a').hasClass('active')){
  jQuery('#gallery_02 li').first().addClass('active');
  colorprop = jQuery('#gallery_02 li').first().attr('data-prop');
} else {
  colorprop = jQuery('#gallery_02 li').attr('data-prop');
}
combinedprop = sizeprop + ';' + colorprop; 

//check if the property is present in the list
jQuery('.data-hidden ul li').each(function(){ 
 if( jQuery(this).attr('data-properties') == combinedprop ){
...
jQuery('#gallery_02 li').on('click', function(){
 jQuery('#gallery_02 li').each(function(){ jQuery(this).removeClass('active'); });
 jQuery(this).addClass('active');
});
</script>
...
$('.service-slider .owl-nav .owl-prev').on("click", function(event) { 

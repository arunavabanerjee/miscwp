	    ...
	    <?php if( $_REQUEST['s'] != '' &&  $_REQUEST['s'] != 'Work_Injury' ) { ?>
          ...
          <?php 
	       ...
	       if(strstr($_REQUEST['s'],'new')){
             $total_items_n = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE `export_type` = 'new'");
...
if($format == 'standard'){
   ...
   $files = explode(',',$record[0][32]);	
   foreach($files as $file){ $name = str_replace('.pdf','',$file);
	 $body .= '<a href="'.wp_upload_dir()['baseurl'].'/policy_uploads/'.$record[0][32].'">'.$name.'</a>';
   } 
...
function generatePolicyPDFs($applicationID, $importType){ 
   ...
   if($applicationID == NULL){ //generate test pdf 
      ...
      if(($record[0][5] == 'S5A') || ($record[0][5] == 'S5B') || ($record[0][5] == 'S5C') || ($record[0][5] == 'S5D') || ($record[0][5] == 'S5AP') || ($record[0][5] == 'S5BP') || ($record[0][5] == 'S5CP') || ($record[0][5] == 'S5DP') ){ 
		//policy jacket
        ...
        $pdffile = $policyDir.$record[0][0].'_Maid_Jacket.pdf'; 
		$pdffileUrl = $record[0][0].'_Maid_Jacket.pdf';
	    ...
	    $pdffileUrl .= ','.$record[0][0].'_Maid_Insurance_Proposal_Form.pdf'; //echo $pdffileUrl;
        $pdf->Output('F',$pdffile); 
      ...
      if(($record[0][5] == 'S2A') || ($record[0][5] == 'S2D')){ 
	    ...
	    $pdffileUrl = $record[0][0].'_Philippine-Embassy-7000-Bond-Proposal-Form.pdf';
            $pdf->Output('F', $pdffile); 
	    //insert record into db
            $interimrecord = $wpdb->get_results("SELECT * FROM $table_name WHERE id = $applicationID", ARRAY_N);  
	        $pdf_filename = $pdffileUrl;
	    ...
	    $pdffileUrl = $record[0][0].'_Philippine-Embassy-2000-Bond-Proposal-Form.pdf';
            $pdf->Output('F', $pdffile);
	    //insert record into db
            $interimrecord = $wpdb->get_results("SELECT * FROM $table_name WHERE id = $applicationID", ARRAY_N);  
	        $pdf_filename = $pdffileUrl;
      ...
      if( $record[0][5] == 'FWMI' ){ 
        ...
        $pdffile = $policyDir.$record[0][0].'_FWMI_Jacket.pdf'; 
		$pdffileUrl = $record[0][0].'_FWMI_Jacket.pdf';
        $pdf->Output('F',$pdffile);

...
cursor: pointer;">X</h2>	
		...
		<div class="col-md-2 col-sm-2 col-xs-2 left-panel"><h2 class="shop-title" style="font-weight:bold;"><?php echo $navi["title"]; ?></h2></div>
		<div class="col-md-10 col-sm-10 col-xs-10 right-panel div-ppath" style="height:50px;overflow:hidden;">
		<?php if(count($navi["item"]) > 10 ){ ?>
                    ...
                    <h3 class="less-btn" style="font-size: 10px;text-align: right;margin-top: -3px;margin-bottom: 0px;width:3%;float:right;">- Less</h3> 
                <?php } ?>
		...
		<?php foreach($navi["item"] as $navValue){ ?>
		 ...
		 <?php if($key != "adv"){ ?>
		 <input type="checkbox" id="<?php echo $navValue["param_value"]; ?>" name="<?php echo $navValue["param_value"]; ?>" value="<?php echo $navValue["param_value"]; ?>" />
		
		 <?php echo $navValue["title"]; ?>
		 <?php } else { ?>
		...
		<div class="select-row">
                <div class="col-md-2 col-sm-2 col-xs-2 left-panel"><h2 class="shop-title" style="font-weight:bold;"><?php echo $navi["title"]; ?></h2></div>
		<div class="col-md-10 col-sm-10 col-xs-10 right-panel div-cat" style="height:50px;overflow:hidden;">
                <?php if(count($navi["item"]) > 10 ){ ?><h3 class="more-btn" style="font-size: 10px;text-align: right;margin-top: -3px;margin-bottom: 0px;">+ More</h3><h3 class="less-btn" style="font-size: 10px;text-align: right;margin-top: -3px;margin-bottom: 0px;width:3%;float:right;">- Less</h3> <?php } ?>
		...
		<li class="product-item flash1 style1 col-md-3 col-sm-6 col-xs-6 col-ts-12 post-1378 product type-product status-publish  instock shipping-taxable product-type-simple">
    ...
    //jQuery(".more-btn").click(function(){
     ...
     //jQuery(".div-cat").toggleClass("increaseHeight", 1000, "easeOutSine");
        ...
        $('.allLessTxt').hide();        
         ...
         $(".div-ppath input").click(function() {
         		$('.div-cat').removeClass('increaseHeight');
			    $('.div-ppath').removeClass('increaseHeight');
			    $(this).parent().closest('div').addClass('increaseHeight');
			    

			    var $path2 = $(this)
			    var $path = $(this).parent().closest('div').addClass('increaseHeight').children( ".more-btn" )	    
			    var $path3 = $(this).parent().closest('div').addClass('increaseHeight').children( ".less-btn" )	    

			    $path.hide()
			    $path3.show().click(function() {
			    	$(this).parent().closest('.div-ppath').attr('class' , 'col-md-10 col-sm-10 col-xs-10 right-panel div-ppath')
			    	$(this).hide()
			    	$path.show()
			    });
         ...
         $(".div-cat input").click(function() {
			    ...
			    $path.hide()
			    $path3.show().click(function() {
			    	$(this).parent().closest('.div-cat').attr('class' , 'col-md-10 col-sm-10 col-xs-10 right-panel div-cat')
			    	$(this).hide()
			    	$path.show()
			    });
         ...
         $(".more-btn").click(function() {
			    ...
			    $(this).hide();
			    $(this).next('.less-btn').show().click(function() {
			    	$(this).parent().closest('.div-ppath').attr('class' , 'col-md-10 col-sm-10 col-xs-10 right-panel div-ppath')
			    	$(this).parent().closest('.div-cat').attr('class' , 'col-md-10 col-sm-10 col-xs-10 right-panel div-cat')
			    	$(this).hide()
			    	$path2.show()
...
<style type="text/css">
	.div-ppath .more-btn{ display: block !important; }
	.div-ppath .less-btn{ display: none; }

	.div-cat .more-btn{ display: block !important; }
	.div-cat .less-btn{ display: none; }
	.increaseHeight .less-btn{ display: block; }
	.increaseHeight .more-btn{ display: none !important; }
     ...
     //jQuery(".div-cat").toggleClass("increaseHeight", 1000, "easeOutSine");
        ...
        $('.allLessTxt').hide();        
         ...
         $(".div-cat").click(function() {
         		$('.div-ppath').removeClass('increaseHeight');
			    $('.div-cat').removeClass('increaseHeight');
			    $(this).addClass('increaseHeight');

			    var $path = $(this).children( ".more-btn" )	    

			    
			    $('.more-btn').removeClass('increaseHeight');
				$path.addClass("increaseHeight");

		        
		            //$(this).find(".more-btn").html("+More");
		            //$(this).find(".increaseHeight").html("-Less");
...
<?php $orgprice = array(); $saleprice = array(); 
      //echo $pdata['item']['orginal_price']; echo $pdata['item']['price']; ?> 
<?php if(strstr($pdata['item']['orginal_price'], '-')){ 
        $orgprice = explode('-',$pdata['item']['orginal_price']); 
        $saleprice = explode('-',$pdata['item']['price']);
      } else {
	array_push( $orgprice, $pdata['item']['orginal_price'] );
	array_push( $saleprice, $pdata['item']['price'] );  
      }
?>
<?php if ( round($orgprice[0], 2) > round($saleprice[0], 2) ){ ?>
...
<?php $selval = count($pdata['item']["item_imgs"]); $maxcount = ceil($selval / 2); //echo $maxcount; ?>
<?php foreach( $pdata['item']["item_imgs"] as $key=>$itemImg ) { //echo $key; ?> 
<?php if( $key == $maxcount ){ break; } ?>
 ...
 <p class="shipping" style="margin-top:10px;">
   <span class="orig-price">
     <span class="woocommerce-Price-currencySymbol">¥</span>&nbsp;
      <?php if(strstr($pdata['item']['price'], '-')){ echo number_format($saleprice[0], 2).' - '.number_format($saleprice[1], 2); ?>
      <?php } else { echo number_format($pdata['item']['price'], 2); } ?>
   </span>
   <ins>
   <?php if ( round($orgprice[0], 2) > round($saleprice[0], 2) ){ ?>
    <span class="dis-price">
      <span class="woocommerce-Price-currencySymbol">¥</span>&nbsp;
       <?php if(strstr($pdata['item']['orginal_price'], '-')){ echo number_format($orgprice[0], 2).' - '.number_format($orgprice[1], 2); ?>
       <?php } else { echo number_format($pdata['item']['orginal_price'], 2); } ?> 
    </span>
   <?php } ?>
 ...
 <?php if($cnt == 1){ $propIndx = $keyArr[0]; $valArr = explode(':',$value); ?>
  <p style="margin-bottom:3px;font-weight:800;"><?php echo $valArr[0]; ?></p><ul>
    <?php if( array_key_exists($key,$pdata['item']["props_img"])) { ?>
     ...
     <?php } else { ?>
     <li class="pdn-text" data-prop="<?php echo $key;?>" style="border:1px solid #000;float:left;margin:3px;padding:1px;list-style:none;width:7%;height:50px;text-align:center;font-size:10px;color:#f00;"><?php echo $valArr[1]; ?></li>
     <?php } ?>
    <?php } else { ?>
     <li class="pdn-text" style="border:1px solid #000;float:left;margin:3px;padding:10px 0;list-style:none;width:7%;height:50px;text-align:center;font-size:11px;" data-prop="<?php echo $key; ?>"><?php echo $valArr[1]; ?></li>
    <?php }} ?>
 <?php } else { // if propIndex is the same ?>
  <?php $valArr = explode(':',$value); ?>
    <?php if( array_key_exists($key,$pdata['item']["props_img"])) { ?>
     ...
     <?php } ?>
    <?php } else { ?>
     <li class="pdn-text" style="border:1px solid #000;float:left;margin:3px;padding: 5px;list-style: none;width: auto;height:50px;text-align:center;font-size:11px;" data-prop="<?php echo $key; ?>"><?php echo $valArr[1]; ?></li>
    <?php } ?>
 <?php } $cnt++; } ?> 
 <?php if($propIndx != 0){ ?></ul><div style="clear:both;margin-bottom:10px;"></div><?php } ?>    
 </div>
...
jQuery('.prod-props ul li').click(function(){ 
  ...
  var mainphtml = jQuery(this).parent().parent(); //console.log(mainphtml);
  ...
  jQuery(mainphtml).find('ul').each(function(){ 
    var ulphtml = jQuery(this); //console.log(ulphtml);
    jQuery(ulphtml).find('li').each(function(){  //alert(jQuery(this).attr('style'));
     //if( jQuery(this).css('border') == '2px solid rgb(255, 150, 51)' ){
     // nskuProp += jQuery(this).attr('data-prop') + ';'; }
     var listyle = jQuery(this).attr('style');
     if( listyle.indexOf('2px solid rgb(255, 150, 51);') > -1 ){
       nskuProp += jQuery(this).attr('data-prop') + ';';
     }
    });
  }); //alert(nskuProp);

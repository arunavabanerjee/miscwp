...
class Custom_Table_Example_List_Table extends WP_List_Table
{
	...
	<button class="button" id='bulkactionbtnPDF' name='bulkactionbtnPDF' value='export_new'>GeneratePDF</button>
	<?php if($_REQUEST['s'] != '' && $_REQUEST['s'] != 'Work_Injury'){ ?> 
          <?php 
	   if(strstr($_REQUEST['s'],'Maid_Embassy')){
             $total_items_n = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE `ins_class` LIKE 'S%' AND `export_type` = 'new'");
	   ...
	   if(strstr($_REQUEST['s'],'Foreign_Worker')){
             $total_items_n = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE `ins_class` LIKE 'FW%' AND `export_type` = 'new'");
	   ...
	   if(strstr($_REQUEST['s'],'Work_Injury')){
             $total_items_n = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE `ins_class` LIKE 'WC%' AND `export_type` = 'new'");
	   ...
	   <button class="button" id='bulkactionbtnExpP' name='bulkactionbtnExpP' value='export_pending' <?php if($total_items_n == 0){ echo "disabled='disabled'"; } ?>>Export Pending (<?php echo $total_items_p; ?>)</button> 
	<?php } ?>
        <?php /*$insclasses = $wpdb->get_results('select distinct ins_class from '.$table_name.' where ins_class in (
			"FWMI","S5A","S5B","S5C","S5D","S5AP","S5BP","S5CP","S5DP","S2A","S2D","WCS") order by 1 asc', ARRAY_A);*
...
{
    global $wpdb;
    $table = new Custom_Table_Example_List_Table(); //var_dump($_REQUEST);

    if(isset($_REQUEST['bulkactionbtnPDF'])){ generatePolicyPDFs(NULL); }
...
function export_applications($insclass, $export_type){
   ...
   if($insclass == 'Maid_Embassy'){
     $items = $wpdb->get_results("SELECT * FROM $table_name WHERE `ins_class` LIKE 'S%' AND `export_type` ='".$export_type."' ORDER BY id DESC", ARRAY_A);
   } elseif($insclass == 'Foreign_Worker'){
     $items = $wpdb->get_results("SELECT * FROM $table_name WHERE `ins_class` LIKE 'FW%' AND `export_type` ='".$export_type."' ORDER BY id DESC", ARRAY_A);
   } elseif($insclass == 'Work_Injury'){
     $items = $wpdb->get_results("SELECT * FROM $table_name WHERE `ins_class` LIKE 'WC%' AND `export_type` ='".$export_type."' ORDER BY id DESC", ARRAY_A);
   }
   //var_dump($items);
   ...
   if($insclass == 'Foreign_Worker'){ $filename = "flweb.txt"; }
   elseif($insclass == 'Work_Injury'){ $filename = "flweb.txt"; }
    ...
    if(isset($_FILES['upload_web_ibms']['name']) && ($_FILES['upload_web_ibms']['error'] != UPLOAD_ERR_NO_FILE)){
	  ...
	  while(true) { $lineNo++;
	    ...
	    if($inserted_Id != '') { 
	      ...
	      generatePolicyPDFs($inserted_Id);
	    }
	  } //end while
    } //if ibms file upload
   } //end if verify nonce
 ...
 *
 * @param $format as standard/wic  
 * @param $id as appln id
 */
 ...
 * @param applicationID
 */
function generatePolicyPDFs($applicationID){ 
   global $wpdb;
   $table_name = 'ins_policy_applicants';
   ...
   if($applicationID == NULL){ //generate test pdf 
      ...
      if($record[0][5] == 'S5A'){ $pdf->Text(112,113,'x'); $pdf->Text(112,117,'x'); } 
      if($record[0][5] == 'S5B'){ $pdf->Text(125,113,'x'); $pdf->Text(112,117,'x'); }
      if($record[0][5] == 'S5C'){ $pdf->Text(135,113,'x'); $pdf->Text(112,117,'x'); }
      if($record[0][5] == 'S5D'){ $pdf->Text(145,113,'x'); $pdf->Text(112,117,'x'); }
      $pdf->AddPage(); $tplIdx2 = $pdf->importPage(2); $pdf->useTemplate($tplIdx2, 0, 0, 200);
      $pdffile = $policyDir.$record[0][0].'_Maid_Insurance_Proposal_Form.pdf'; 
      $pdf->Output('F',$pdffile);*/ 

      //domestic maid insurance - Philippines Embassy 2000 Bond Proposal Form
      ...
      $pdffile = $policyDir.$record[0][0].'_Philippine-Embassy-2000-Bond-Proposal-Form.pdf'; 
      $pdf->Output('F',$pdffile);*/

      //domestic maid insurance - Philippines Embassy 7000 Bond Proposal Form
      ...
      $pdffile = $policyDir.$record[0][0].'_Philippine-Embassy-7000-Bond-Proposal-Form.pdf'; 
      $pdf->Output('F',$pdffile);  */

...
if( homey_is_renter() ) {
    ...
    if(homey_is_admin()) {
        if(isset($_GET['mine']) && $_GET['mine'] == 1) {
            ...
            $meta_query[] = array(
                'key' => 'reservation_status',
                'value' => array('cancelled','completed','declined'),
                'compare' => 'IN'
            );
            $meta_query['relation'] = 'AND';
    } else {
        ...
        <div class="container-fluid">
            <div class="row">
				...
				<?php if($current_user->roles[0] == 'homey_brokers' || $current_user->roles[0] == 'homey_agents'){ ?>
                                <h2 class="title" style="width:75%;float:left;"><?php echo esc_html__('Manage Leads'); ?></h2> 
				  <?php if($current_user->roles[0] == 'homey_brokers') { ?>
				   <button type="button" id="allocate-reservation" name="allocate-reservation" class="btn btn-primary"  style="float:right;height:35px;line-height:1px;padding:0 8px;"><?php echo esc_html__('Allocate Leads Via Rules'); ?></button>
				  <?php } ?>
				<?php } else { ?>
				...
				<?php } ?>
					    ...
					    <th><?php echo esc_html__('Received On'); ?></th>

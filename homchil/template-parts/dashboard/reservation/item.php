    ...
    <td data-label="<?php echo esc_attr($homey_local['actions_label']); ?>">
	    ...
	    $btnstyle = 'style="padding:0px 8px;margin:2px;"';
		    ...
		    if($dashboarduser_role == 'homey_agents'){
		      ...
		      if($reservation_status == 'under_review'){ 
			if(empty($reservation_status_ag) || $reservation_status_ag != "rejected"){
			   ...
			   echo '<button type="button" name="new-lead-agent-accept" class="btn btn-full-width" 
				style="font-size:11px;font-weight:bold;border:1px solid #f15e75;background-color:#f15e75;">'.esc_html__('Accept').'</button>';
			}
 		      }
		    }
		    ...
		    if($dashboarduser_role == 'homey_brokers'){ 
		      ...
		      /*$user_args  = array( 'role__in' => ['homey_agents'], 'orderby' => 'display_name' );
   	              $agents = get_users($user_args);
                      echo "<select class='homey-selagents' name='homey-agents-brokers-dashboard' style='width:75%;'>";

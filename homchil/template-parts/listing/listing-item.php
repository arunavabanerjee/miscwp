...
<div class="item-wrap infobox_trigger homey-matchHeight">
    <div class="media property-item">
        ...
        <div class="media-body item-body clearfix">
		    ...
		    <?php  if(!empty($address)) { echo '<address class="item-address">'.esc_attr($address).'</address>'; } ?>
		    <?php } else { ?>
		    <?php if(!empty($address)) { $address_array = explode(',',$address); 
			   echo '<address class="item-address">'.esc_attr($address_array[sizeof($address_array)-1]).'</address>'; ?>
		    <?php }} ?>
		    <?php } else { ?>
		    <?php if(!empty($address)) { $address_array = explode(',',$address); 
			   echo '<address class="item-address">'.esc_attr($address_array[sizeof($address_array)-1]).'</address>'; ?>

...
$jq(document).ready(function(){
	...
	var login_sending = HOMEY_ajax_vars.login_loading;
	    ...
	    //alert('form ok submitting');
			...
			'action' : 'homey_child_add_new_agent_submission',
                ...
                success: function(response) {
                    if( response.success ) {
                        $topmessages.empty().append(response.msg);
                        $bottommessages.empty().append(response.msg);
                        $form.find('input').val('');
                        //$form.find('textarea').val('');
                    } else {
                        $topmessages.empty().append(response.msg);
	...
	$jq('.selected-houses-host-result').on('click', '.denied', function(){ 
         ...
         * -------------------------------------------------------------------------*/
        $jq('.custom-actions-addln select').on('change', function(e) { //alert('this');
            e.preventDefault();
	    ...
	    //alert($retrieved_ids); return false;
			...
			'action' : 'homey_child_property_agent_allocation',
                ...
                success: function(response) {
                    if( response.success ) {
                        $messages.remove('i');
                        $messages.empty().html('<span style="color:#00FF00">Agent Assigned!!</span>');
			$jq('.custom-actions-addln select').val("0");
			setTimeout(window.location.reload(),3000);
                    } else {
                        $messages.remove('i');
                        $messages.empty().append('<span style="color:#00FF00">Agent CouldNot Be Assigned</span>');
                    }
                    if(homey_reCaptcha == 1) {
                        homeyReCaptchaReset();
                    }
        ...
        /*--------------------------------------------------------------------------
         *  Reservation status As Assigned By Agent
         * -------------------------------------------------------------------------*/

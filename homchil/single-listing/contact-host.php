...
if($current_user->roles[0] == "homey_brokers" || $current_user->roles[0] == "homey_agents"){ 
  $sendingUName = $current_user->display_name; 
  $sendingUEmail = $current_user->user_email;
        ...
        <div class="modal-body">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><?php echo esc_attr($homey_local['pr_cont_host']).' For Property Id: '.$post->ID; ?></h4>
                </div>
			    ...
			    <input type="hidden" name="postcode" value="<?php echo get_post_meta($post->ID, 'homey_zip', true); ?>"/>
                            <input type="hidden" name="action" value="homey_host_contact">
                            <div class="form-group">
                                <input type="text" name="name" class="form-control" placeholder="<?php echo esc_attr($homey_local['con_name']); ?>" <?php if(isset($sendingUName)){ echo 'value="'.$sendingUName.'"'; } ?>>
                            </div>
                            <div class="form-group">
                                <input type="email" name="email" class="form-control" placeholder="<?php echo esc_attr($homey_local['con_email']); ?>" <?php if(isset($sendingUEmail)){ echo 'value="'.$sendingUEmail.'"'; } ?>>
                            </div>
                            <div class="form-group">
			    ...
			    <button type="submit" class="contact_listing_host_ajax btn btn-primary btn-full-width"><?php echo esc_attr($homey_local['submit_btn']); ?></button>
                        </form>

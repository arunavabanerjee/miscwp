...
class Custom_Table_Example_List_Table extends WP_List_Table
{
	...
	$move_on_url = '&s=';
        ...
        <?php //$insclasses = $wpdb->get_results('select distinct ins_class from '.$table_name.' where ins_class in ("FWMI","S5A","S5B","S5C","S5D","S5AP","S5BP","S5CP","S5DP","S2A","S2D","WCS","WIC") order by 1 asc', ARRAY_A);
	  $insclasses = array('Maid_Embassy', 'Foreign_Worker', 'Work_Injury');
          if( $insclasses ){ ?>
            <select id="inscl-filter" name="inscl-filter">
                ...
                <?php foreach( $insclasses as $insclass ){ $selected = ''; 
		...
		<?php } */?>
                <?php if($_REQUEST['s'] == $insclass){ $selected = ' selected = "selected"'; ?>
                    <option value="<?php echo $move_on_url .$insclass; ?>" <?php echo $selected; ?>><?php echo $insclass; ?></option>
                <?php } else { ?>
                    <option value="<?php echo $move_on_url .$insclass; ?>"><?php echo $insclass; ?></option>
		<?php } ?>
		<?php } //end foreach ?>
            </select>
         <?php } ?>  
...
<div class="wrap">
	...
	<!--<a class="add-new-h2" href="<?php //echo get_admin_url(get_current_blog_id(), 'admin.php?page=persons_form');?>">
		<?php //_e('Add new', 'custom_table_example')?></a>-->
    </h2>
    <?php echo $message; ?>
...
//add_action( 'restrict_manage_posts', 'custom_table_filter_insurance_class' );
...
/*function custom_table_filter_insurance_class()
{
    global $wpdb;
    $table_name = 'ins_policy_applicants';
    //$sql = 'SELECT DISTINCT meta_key FROM '.$wpdb->postmeta.' ORDER BY 1';
    $sql = 'SELECT DISTINCT ins_class FROM '.$table_name.' ORDER BY 1';
    $fields = $wpdb->get_results($sql, ARRAY_N);
?>
<select name="appln_class">
<option value=""><?php _e('Filter: Insurance Class', 'custom_table_example'); ?></option>
<?php
    $current = isset($_GET['appln_class'])? $_GET['appln_class']:'';
    //$current_v = isset($_GET['appln_class'])? $_GET['appln_class']:'';
    foreach ($fields as $field) {
      if (substr($field[0],0,1) != "_"){
        printf ('<option value="%s"%s>%s</option>',$field[0], $field[0] == $current? ' selected="selected"':'', $field[0] );
      }
    }
?>
</select> 
...
/**
 * Addition of Functionality for the views segment
 *
 * describe the filter that generates views functionality
 * ========================================================
 */
/*add_filter('views_toplevel_page_applications','my_plugin_slug_status_links',10, 1);
function my_plugin_slug_status_links($views) {
    global $pagenow;
   //$views['scheduled'] =  "<a href='#'>Scheduled</a>";

   $views['scheduled'] =  "<a href='".esc_url(add_query_arg('mode',$views['scheduled']))."'>Scheduled</a>";
   return $views;
}*/
...
function custom_table_example_upload_form_page_handler()
{
	...
	wp_die();
	 ...
	 //check whether both files have same records 
	 $StatFileArr = file($_FILES['upload_web_stat']['tmp_name']); 
	 $IBMSFileArr = file($_FILES['upload_web_ibms']['tmp_name']); 
	 foreach($StatFileArr as $lineNo => $statline){ 
	   if($lineNo == 0){ continue; }
	   //get values from web stat file
	   $wstatsysID = trim(substr($statline,0,7)); $otherID = trim(substr($statline,7,18));
	   $wstatguarantee_no = trim(substr($statline,25,21)); $wstatmaster_policy_no = trim(substr($statline,46,12));
	   $ibmsline = $IBMSFileArr[$lineNo];
	   if(stristr($ibmsline, $wstatguarantee_no) && strstr($ibmsline, $wstatmaster_policy_no)){ continue; }
	   else{	    
	      $errmessage = '<h3 style="background:#f3123acc;color:#000;font-size:13px;">';
	      $errmessage .= 'Error: Web Stat & Web IBMS File do not match records.'; 
	      $errmessage .= '<br/> Web Stat Line No: '.$lineNo.' & Web IBMS Line No: '.($lineNo-1);	      
	      $errmessage .= '</h3>';
	      echo $errmessage;
	      echo '<a class="button" href="'.get_admin_url(get_current_blog_id(), 'admin.php?page=upload_form').'">';
	      echo _e('Back To Upload', 'custom_table_example'); echo '</a>';
	 ...
	 while(true) {
	   ...
	   if(count($results_id) == 1){  
	     ...
	     //var_dump($updt_result);
	     //if($results_id[0][0] != '') { $notice .= __('Item Updated: '//.$results_id[0][0].'<br/>', 'custom_table_example'); }
	     if($results_id[0][0] != '') { 
	         $notice .= __('Item Updated: '.$results_id[0][0].'<br/>', 'custom_table_example'); 
	         $to = $results_id[0][31];
             $subject = 'Generation of Policy Number and Invoice Number - Appln ID -'.$results_id[0][0];
             $body = 'Hello, <br/> Please note that the Policy Number and Invoice Number for the application of '.$results_id[0][21].' have been generated. <br/> Rgds.';
             $headers[] = 'Content-Type: text/html; charset=UTF-8';
             $headers[] = 'From: Webmaster <webmaster@sketchwebsolutions.in>';
...
function custom_table_example_application_viewform_page_handler()
{
    ...
    //var_dump($_REQUEST); var_dump($_POST); //var_dump($_FILES);

...
function filter_woocommerce_add_cart_item_data( $cart_item_data, $product_id ) {
  ...
  if($policyidsStr == ''){ 
	  $policyidsStr = $_REQUEST['pl'].','; 
	  WC()->session->set('policyids', $policyidsStr);
  }
  $policyids = explode(',',$policyidsStr); //var_dump($policyids); 
  $removedCartItems = WC()->session->get( 'removed_cart_contents'); //var_dump($removedCartItems);
  $items = WC()->cart->get_cart(); //var_dump($items); exit; 
  
  foreach($policyids as $policyid){ //echo $policyid;
    if($policyid != ''){ $flag = 0;
    //check if policy in removed cart contents
    foreach($removedCartItems as $rkey => $rItem){
      if($rItem['plan-id'] == $policyid){ $flag = 1; break; }
    }
...
function action_woocommerce_before_calculate_totals( $cart ) { //var_dump(WC()->session);
  global $wpdb; 
  $removedCartItems = WC()->session->get( 'removed_cart_contents'); //var_dump($removedCartItems);
  foreach ( $cart->cart_contents as $key => $value ) { //var_dump($key); //var_dump($value); 
   if(!isset($value["plan-id"])){ $cart->remove_cart_item($key); }
    ...
    //set title
    if($value['plan-selected'] == 'WCS'){ $_wcproduct->set_name('Work Injury Compensation'); }
    if($value['plan-selected'] == 'FWMI'){ $_wcproduct->set_name('Foreign Worker Medical'); }
    if($value['plan-selected'] == 'S2A' || $value['plan-selected'] == 'S2D'){ 
		$_wcproduct->set_name('Phillipines Embassy Bond'); 
	}
    if($value['plan-selected'] == 'S5A' || $value['plan-selected'] == 'S5B' || 
	$value['plan-selected'] == 'S5C' || $value['plan-selected'] == 'S5D' ){ 
		$_wcproduct->set_name('Domestic Maid'); 
	}
...
add_action('woocommerce_cart_totals_before_order_total', 'action_woocommerce_cart_totals_before_order_total');
function action_woocommerce_cart_totals_before_order_total(){ //var_dump(WC()->session);
  global $wpdb; //$cart = WC()->cart; var_dump($cart); 
  $policyids = WC()->session->get('policyids'); //var_dump($policyids); 
  $policyarray = explode(',',$policyids); $policyarray = array_filter($policyarray);
  //$policyids = implode(',',$policyarray);
  $removedCartItems = WC()->session->get( 'removed_cart_contents'); //var_dump($removedCartItems);
  
  //query to fetch the data from db 
  $total_premium = 0.0; $total_gstval = 0.0;
  foreach($policyarray as $policyid){ $flag =0;
    foreach($removedCartItems as $rkey => $rItem){
      if($rItem['plan-id'] == $policyid){ $flag = 1; break;  }
    }
    if($flag == 0){
    ...
    //if set post_id
    if($post_id){ 			  
      $total_premium += $post_id->Total_premium_amt; 
      //multiple policy like fwmi, wcs
      if($postall_id){ 
        foreach($postall_id as $record){ $total_premium += $record->Total_premium_amt; }
      } //echo $total_premium;
      
      if(!empty($post_id->gstamt)){ 
         //$post_id->gstamt."<br>";
         //echo "TGST-".$total_gstval += ($post_id->gstamt * $total_premium) / 100;
         $gstval = $post_id->gstamt/100;
         $indv_gstval = number_format(($post_id->Total_premium_amt * $gstval), 2); //echo " GST Indv: ". $indv_gstval;
         //$indv_gstval = $post_id->Total_premium_amt * $gstval; echo " GST Indv: ". $indv_gstval;
         $total_gstval += $indv_gstval; //echo " GST Total: ". $total_gstval; echo '<br/>';
                  
         if($postall_id){ 
           foreach($postall_id as $record){ 
             $indv_gstval = number_format(($record->Total_premium_amt * $gstval), 2); //echo " GST Indv: ". $indv_gstval;
             $total_gstval += $indv_gstval; //echo " GST Total: ". $total_gstval; echo '<br/>'; 
           }
         }
      }
  ...
  //if applied coupon, then put the discounted amount
  if(!empty(WC()->session->get('coupon_discount_totals'))){
    $discountInfo = WC()->session->get( 'coupon_discount_totals' );   
    $post_id = $wpdb->get_row("SELECT * from ins_policy_applicants where id=$policyid"); //print_r($post_id);
    $postall_id = $wpdb->get_results("SELECT * from ins_policy_applicants where insumem_ID=$policyid"); 
    $total_premium_1 = 0.0; $discounted_premium = 0.0;
    
    //get total value of price 
    if($post_id){ 			  
      $total_premium_1 += $post_id->Total_premium_amt; 
      //multiple policy like fwmi, wcs
      if($postall_id){ foreach($postall_id as $record){ $total_premium_1 += $record->Total_premium_amt; }} 
      //echo $total_premium;
    }  
    //apply coupon on total premium
    foreach($discountInfo as $key=>$value){ 
       //get the discount info for each coupon
       $cpInfo = new WC_Coupon($key); //var_dump($cpInfo);
       if($cpInfo->discount_type == 'percent'){
          $discounted_premium +=  number_format(($total_premium_1 - ($cpInfo->amount/100 * $total_premium_1)), 2);
       }
    }
    if($discounted_premium != 0.0){ 
       $total_premium = $discounted_premium; 
       $total_gstval = number_format(($total_gstval - ($cpInfo->amount/100 * $total_gstval)), 2);
    }
  }
  
  echo '<tr class="cart-pre-subtotal"><td>Premium:</td><td>'.get_woocommerce_currency_symbol().sprintf('%0.2f',$total_premium).'</td></tr>';
  echo '<tr class="cart-pre-subtotal"><td>GST:</td><td>'.get_woocommerce_currency_symbol().sprintf('%0.2f',$total_gstval).'</td></tr>';
}

/*add_action('woocommerce_remove_cart_item', 'action_woocommerce_remove_cart_item');
function action_woocommerce_remove_cart_item($rkey) { 
  echo 'key-'; var_dump($rkey); var_dump(WC()->session);
  global $wpdb; $cart = WC()->cart; //var_dump($cart);
  $policyids = WC()->session->get('policyids'); //var_dump($policyids); 
  $policyarray = explode(',',$policyids); $policyarray = array_filter($policyarray);
  //$policyids = implode(',',$policyarray);
  foreach ( $cart->cart_contents as $key => $value ) { var_dump($key); //var_dump($value);
    if($key == $rkey){ $planId = $value["plan-id"];
        $policyids = WC()->session->get('policyids'); //var_dump($policyids); 
        $policyarray = explode(',',$policyids); $policyarray = array_filter($policyarray); 
        //$policyids = implode(',',$policyarray);
        foreach($policyarray as $arrkey => $policyid){ 
          if($policyid ==  $planId){ unset($arrkey); } 
		}
        $policyids=""; $policyids = implode(',',$policyarray); 
        WC()->session->set('policyids', $policyids);
	}
  }
}*/

/*add_action('woocommerce_cart_item_removed', 'action_woocommerce_cart_item_removed');
function action_woocommerce_cart_item_removed($rkey) { echo 'key'; var_dump($rkey); var_dump(WC()->session); 
  global $wpdb; $cart = WC()->cart; //var_dump($cart);
  $policyids = WC()->session->get('policyids'); //var_dump($policyids); 
  $policyarray = explode(',',$policyids); $policyarray = array_filter($policyarray);
  //$policyids = implode(',',$policyarray);
  foreach ( $cart->cart_contents as $key => $value ) { var_dump($key); //var_dump($value);
    if($key == $rkey){ $planId = $value["plan-id"]; var_dump($planId);
      $policyids = WC()->session->get('policyids'); //var_dump($policyids); 
      $policyarray = explode(',',$policyids); $policyarray = array_filter($policyarray); 
      //$policyids = implode(',',$policyarray);
      foreach($policyarray as $arrkey => $policyid){ 
...
function func_woocommerce_set_input_attrs( $fields ) { 
  ...
  if(!is_user_logged_in()){
    ...
    //order comments
    //$fields['order']['order_comments']['default'] = 'Policy IDs:'.$policyids;
  }
  //check for multiple ids if submissions
  foreach($policyarray as $policyid){ $flag =0;
    foreach($removedCartItems as $rkey => $rItem){
      if($rItem['plan-id'] == $policyid){ $flag = 1; break; }
    }
    if($flag == 0){
     $post_id = $wpdb->get_row("SELECT * from ins_policy_applicants where id=$policyid");//print_r($post_id);
     $postall_id = $wpdb->get_results("SELECT * from ins_policy_applicants where insumem_ID=$policyid");
     if($postall_id){ $nplans = array();
      foreach($postall_id as $postal_id){  array_push($nplans, $postal_id->id); } 
      $policyarray = array_merge($policyarray, $nplans);
      $policyids = implode(',',$policyarray);
     }
    }
  }
  //order comments
  $fields['order']['order_comments']['default'] = 'Purchased Policy IDs:'.$policyids;  
  $fields['order']['order_comments']['custom_attributes'] = array('readonly'=>'readonly');  	
  return $fields;
}
...
function add_custom_checkout_hidden_field( $checkout ) {
    ...
    echo '<div id="user_link_hidden_checkout_field">
           <input type="hidden" class="input-hidden" name="policy_ids" id="policy_ids" value="' . $policyids . '">
          </div>';
}
...
function save_custom_checkout_hidden_field( $order_id ) {
    if ( ! empty( $_POST['policy_ids'] ) ) {
        update_post_meta( $order_id, '_policy_ids', sanitize_text_field( $_POST['policy_ids'] ) );
    }
}
...
function display_verification_id_in_customer_order( $order ) {
    ...
    echo '<p class="policy-ids"><strong>'.__('Policy IDs', 'woocommerce') . ':</strong> ' . get_post_meta( $order_id, '_policy_ids', true ) .'</p>';
}
...
function display_verification_id_in_admin_order_meta( $order ) { //var_dump($order);
    // compatibility with WC +3
    $order_id = method_exists( $order, 'get_id' ) ? $order->get_id() : $order->id;
    echo '<p><strong>'.__('Policy Ids', 'woocommerce').':</strong> ' . get_post_meta( $order_id, '_policy_ids', true ) . '</p>';
}
...
function add_verification_id_to_emails_notifications( $order, $sent_to_admin, $plain_text, $email ) {
    ...
    $order_id = method_exists( $order, 'get_id' ) ? $order->get_id() : $order->id;
    $output = '';
    $policy_ids = get_post_meta( $order_id, '_policy_ids', true );
    if ( !empty($policy_ids) )
        $output .= '<div><strong>' . __( "Policy Ids:", "woocommerce" ) . '</strong> <span class="text">' . $policy_ids . '</span></div>';
    echo $output;
}*/
...
function add_final_status_update( $order_id ) {
  ...
  $arr_policyids = array_filter($arr_policyids); 

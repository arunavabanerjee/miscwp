...
function custom_table_example_upload_form_page_handler()
{
    ...
    if(isset($_FILES['upload_web_ibms']['name']) && ($_FILES['upload_web_ibms']['error'] != UPLOAD_ERR_NO_FILE)){
	  ...
	  while(true) { $lineNo++;
	    ...
	    $data = array('userID'=>0, 'entry_date' => $entry_date, 'ins_class' => $ins_class,'er_cpfno' => $er_cpfno,'wp_no' => $wp_no,
	                'eff_date'=> $eff_date, 'exp_date' => $exp_date, 'ins_party_nm' => $ins_party_nm, 'ins_party_nric' => $ins_party_nric, 
	                'addr1' => $addr1, 'addr2' => $addr2, 'contact_person' => $contact_person, 'ph_no' => $ph_no, 'ins_person' => $ins_person, 
	                'dob' => $dob, 'nationality' => $nationality, 'passport' => $passport, 'guarantee_no' => $guarantee_no, 
	                'master_policy_no' => $master_policy_no, 'export_type'=>'completed' );

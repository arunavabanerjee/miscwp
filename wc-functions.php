...
function woocommerce_add_product_slider(){
  ...
  $sliderhtml .= '<div class="clearfix"></div>
	<div class="container"><div class="product-section view-more"><div class="row">
  ...
  endwhile;
  wp_reset_postdata();     
  $sliderhtml .= '</div></div></div></div></div></div><div class="clearfix"></div>';
  echo $sliderhtml;
}
...
function woocommerce_after_shop_loop_item_title_short_description() {
  ...
  $post = get_post($product->post->ID);   
  $descString = substr( $post->post_content, 0, 35 ); 
  //echo '<p class="desc">'.$descString.'</p>';
}
...
add_filter( 'yith_wcwl_add_to_cart_label', 'filter_yith_wishlist_add_to_cart_label', 10, 1 );   
function filter_yith_wishlist_add_to_cart_label( $localize_label ) { 
   $localize_label = 'Order Now'; return $localize_label; 
}; 
add_filter( 'yith_wcwl_button_label', 'filter_yith_wcwl_button_label', 10, 1 ); 
function filter_yith_wcwl_button_label( $localize_label ) { 
    $localize_label = '<i class="fa fa-heart-o" aria-hidden="true"></i> Wishlist';
    return $localize_label; 
};
add_filter( "yit_{$name}_labels", 'filter_yit_name_labels', 10, 1 );
function filter_yit_name_labels( $labels ) { 
    $labels = 'Compare Now'; return $labels; 
}; 
